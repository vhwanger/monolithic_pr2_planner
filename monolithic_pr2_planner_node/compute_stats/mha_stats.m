...
for idx=range
    ...
    %other_methods = [prm_stats rrt_stats rrtstar_stats ara_stats imha_stats];
    other_methods = [prm_stats rrt_stats rrtstar_stats ];
    imha_comparison = compareMethods(imha_stats,other_methods);

    % for each 'other' planner
    [path,planner_name,ext] = fileparts(imha_comparison.method.name)



    %ara_stats2 = computeMethodStatsSBPL([path '/ara_'],num);
    %imha_stats2 = computeMethodStatsSBPL([path '/imha_'],num);
    %sbpl_compare = compareMethodsSBPL(imha_stats2, [ara_stats2]);
    ...
    if ~isfield(success_rate, planner_name)
        success_rate.(planner_name) = sum(imha_stats.base>0);
    else
        success_rate.(planner_name) = success_rate.(planner_name) + sum(imha_stats.base>0);
    end

    other = imha_comparison.other;
