...
BFS2DHeuristic::BFS2DHeuristic(){
    ...
    ROS_DEBUG_NAMED(HEUR_LOG, "[BFS2D] initialized BFS2D of size %d %d", m_size_col, m_size_row);
    m_bfs.reset(new sbpl_bfs_2d(m_size_col, m_size_row, threshold));
    
    // Initialize the grid here itself so that you don't wait for the map callback to be called
...
int BFS2DHeuristic::getGoalHeuristic(GraphStatePtr state){
    ...
    // Check if within bounds. We need to do this here because the bfs2d implementation doesn't take care of this.
    if (obj_state.x() < 0 || obj_state.x() >= m_size_col || obj_state.y() < 0 || obj_state.y() >= m_size_row) {
        ROS_DEBUG_NAMED(HEUR_LOG, "[BFS2D] Out of bounds: %d %d", obj_state.x(), obj_state.y());
        return 0;
    }
